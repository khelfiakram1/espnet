...
# Copyright (C) 2025 Jhu
# (Mohammed Akram KHELFI)

if [ $# -ne 5 ]; then
  echo "Usage: $0 <qasr-dir> <mgb2-dir> <process-xml> <data-subset> <mer>"
  exit 1;
fi
...
mgb2_dir=$2
process_xml=$3
subset=$4  # subset of training data
mer=$5
...
qasr_xmldir=$qasr_dir/release/train_20210109/xml
mgb2_xmldir=$mgb2_dir/train/xml/utf8
...
if [ $process_xml == "python" ]; then
  ...
  cat $train_dir/wav_list | while read basename; do
    ...
    local/process_xml.py $qasr_xmldir/$basename.xml - | local/add_to_datadir.py $basename $train_dir $mer
  done
#------Need to write for process_xml='xml'
else
  # invalid option
...
for x in text segments; do
  cp $mgb2_dir/dev/${x}.all $dev_dir/${x}
done


find $mgb2_dir/dev/wav -type f -name "*.wav" | \
  awk -F/ '{print $NF}' | perl -pe 's/\.wav//g' > \
  $dev_dir/wav_list

for x in $(cat $dev_dir/wav_list); do
  echo $x $mgb2_dir/dev/wav/$x.wav >> $dev_dir/wav.scp
done
...
for list in overlap non_overlap; do
  ...
  for x in segments text utt2spk; do
    utils/filter_scp.pl $mgb2_dir/dev/${list}_speech $dev_dir/$x > ${dev_dir}_$list/${x}
  done
done
...
# Test
cp $mgb2_dir/test/text $test_dir/text
cp $mgb2_dir/test/segments.non_overlap_speech $test_dir/segments


find $mgb2_dir/test/wav -type f -name "*.wav" | \
  awk -F/ '{print $NF}' | perl -pe 's/\.wav//g' > \
  $test_dir/wav_list

for x in $(cat $test_dir/wav_list); do
  echo $x $mgb2_dir/test/wav/$x.wav >> $test_dir/wav.scp
done
...
for list in overlap non_overlap; do
  ...
  for x in segments text utt2spk; do
    utils/filter_scp.pl $mgb2_dir/test/${list}_speech.lst $test_dir/$x > ${test_dir}_$list/${x}
  done
done
